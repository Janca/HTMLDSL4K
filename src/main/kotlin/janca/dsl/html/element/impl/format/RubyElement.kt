...
interface RubyElementProvider : IElement {
    fun ruby(className: String? = null, id: String? = null, text: String? = null, init: (RubyElement.() -> Unit)? = null) = addChild(RubyElement().init(className = className, id = id, text = text, init = init))
}