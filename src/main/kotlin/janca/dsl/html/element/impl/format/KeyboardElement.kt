...
interface KeyboardElementProvider : IElement {
    fun kbd(className: String? = null, id: String? = null, text: String? = null, init: (KeyboardElement.() -> Unit)? = null) = addChild(KeyboardElement().init(className = className, id = id, text = text, init = init))
}