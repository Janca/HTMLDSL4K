...
interface MarkElementProvider : IElement {
    fun mark(className: String? = null, id: String? = null, text: String? = null, init: (MarkElement.() -> Unit)? = null) = addChild(MarkElement().init(className = className, id = id, text = text, init = init))
}