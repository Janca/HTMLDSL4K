...
interface QuoteElementProvider : IElement {
    fun q(className: String? = null, id: String? = null, text: String? = null, init: (QuoteElement.() -> Unit)? = null) = addChild(QuoteElement().init(className = className, id = id, text = text, init = init))
}