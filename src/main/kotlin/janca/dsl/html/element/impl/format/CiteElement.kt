...
interface CiteElementProvider : IElement {
    fun cite(className: String? = null, id: String? = null, text: String? = null, init: (CiteElement.() -> Unit)? = null) = addChild(CiteElement().init(className = className, id = id, text = text, init = init))
}