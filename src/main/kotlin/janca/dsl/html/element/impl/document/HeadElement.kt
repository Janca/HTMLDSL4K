...
interface HeadElementProvider : IElement {
    fun head(className: String? = null, id: String? = null, text: String? = null, init: HeadElement.() -> Unit) =
        addChild(HeadElement().init(className = className, id = id, text = text, init = init))
}