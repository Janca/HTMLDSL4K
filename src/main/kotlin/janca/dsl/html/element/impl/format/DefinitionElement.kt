...
interface DefinitionElementProvider : IElement {
    fun dfn(text: String? = null, className: String? = null, id: String? = null, init: (DefinitionElement.() -> Unit)? = null) = addChild(DefinitionElement().init(className = className, id = id, text = text, init = init))
}