...
interface FooterElementProvider : IElement {
    fun footer(className: String? = null, id: String? = null, text: String? = null, init: (FooterElement.() -> Unit)? = null) = addChild(FooterElement().init(className = className, id = id, text = text, init = init))
}