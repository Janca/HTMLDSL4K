...
interface FigureElementProvider : IElement {
    fun figure(className: String? = null, id: String? = null, text: String? = null, init: (FigureElement.() -> Unit)? = null) = addChild(FigureElement().init(className = className, id = id, text = text, init = init))
}